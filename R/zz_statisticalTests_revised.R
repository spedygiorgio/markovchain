...
verifyEmpiricalToTheoretical <- function(data, object,
  ...
  support <- matrix(informative, nrow = length(states), ncol = length(states), byrow = TRUE)

  if (any(observed[support] > 0 & P[support] == 0)) {
  ...
  positive <- support & (P > 0)
  if (method == "G") {
    statistic <- 2 * sum(observed[positive] * log(observed[positive] / expected[positive]))
  } else {
    statistic <- sum((observed[positive] - expected[positive])^2 / expected[positive])
  }
...
#'   Contingency Tables and Markov Chains. *Technometrics*, 4(4), 573--608.
#' @export
verifyHomogeneity <- function(inputList, method = c("G", "Pearson"), verbose = TRUE) {
  method <- match.arg(method)
  ...
  for (i in seq_along(states)) {
    ...
    for (s in which(active)) {
      ...
      } else statistic <- statistic + sum((obs - exp)^2 / exp)
    }
