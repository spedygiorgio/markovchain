...
For the first three tests, we use the $\chi ^2$ statistics \citep{MSkuriat}. 
...
We first construct a contingency table in which the columns are the frequency of past$\rightarrow$present$\rightarrow$future (PPF), present$\rightarrow$future (PF) and (PF - PPF), and the rows are the possible three state transitions as shown in Table \ref{tab:property} for each possible present to future transition.
...
The \code{assessOrder} function verifies if the given chain is of first order.
...
Using the table, the function performs the $\chi ^2$ test by calling the \code{chisq.test} function.
This test returns a list of the chi-squared value, the degrees of freedom, and the p-value for each state. If the p-value is greater than the given significance level, we cannot reject the hypothesis that the chain is of first order. 
...
Using the table, the function performs the $\chi ^2$ test by calling the \code{chisq.test} function.
This test returns a list of the chi-squared value, the degrees of freedom, and the p-value for each state.
If the p-value is greater than the given significance level, we cannot reject the hypothesis that the chain is stationary.
...
divergenceTest(mcFit$estimate@transitionMatrix
               , mcFit$estimate@transitionMatrix, length(sequence))
@
