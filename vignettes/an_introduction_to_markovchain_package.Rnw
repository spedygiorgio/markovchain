...
This test invloves verifying whether the given sequence holds the Markov property for which the future state is independent of the past states and depends only on the current state.
This test returns a list of the chi-squared value, the degrees of freedom, and the p-value for each possible transition.
...
This test assesses the order of the given sequence.
...
\subsubsection{Assessing the statinarity of a Markov chain sequence}
This test assesses if the transition probabilities change over time. 
This test returns a list of the chi-squared value, the degrees of freedom, and the p-value for each state.
