...
The \code{assessOrder} function verifies if the given chain is of second order.
...
The \code{assessStationarity} function assesses if the transition probabilities of the given chain change over time. 
This test returns a list of the chi-squared value, the degrees of freedom, and the p-value for each state.
