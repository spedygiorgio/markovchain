...
For the first three tests, we use the $\chi ^2$ statistics. 
...
In order to test if two empirically estimated transition matrices $m1$ and $m2$ are different, we use the $\phi$-divergence test \cite{pardo2005statistical} that is given by
