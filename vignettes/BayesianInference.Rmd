...
$$ \hat{\theta} = \Big\{ \frac{n(u, s) + \alpha(u, s)}{n(u) + \alpha(u)}, s \in \mathcal{A}, u \in \mathcal{A} \Big\}  $$

It is noteworthy that $\text{E}_{\text{prior}} p(s|u)$ coincides with the MAP estimate. The variance is given by
...
markovchainFit(weathersOfDays[1:200], method = "map", confidencelevel = 0.92, 
               hyperparam = matrix(c(1, 1, 2,
                                     3, 2, 1,
                                     2, 2, 3), nrow = 3),
               newData = weathersOfDays[201:365])
```
