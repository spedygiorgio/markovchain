...
#' @title Internal helper to map partition names to C++ indices
#' @description Validates the user-provided partition and translates state names 
#' into 0-based integer indices required by the C++ backend.
#' @param state_names Character vector of the original Markov chain states
#' @param partition A named list of character vectors
#' @return A list of integer vectors (0-based)
#' @keywords internal
.get_partition_indices <- function(state_names, partition) {
  
  # 1. Match state names to their positions in the matrix (1-based index)
  part_idx <- lapply(partition, function(x) {
    ...
    return(idx - 1L) # Subtract 1 because C++ arrays are 0-indexed
  })
  
  # 2. Strict validation: The partition must be exhaustive and mutually exclusive.
  # We check that the total number of elements matches the matrix dimension,
  # and that there are no duplicate states across different macro-states.
  all_idx <- unlist(part_idx)
  if (length(all_idx) != length(state_names) || length(unique(all_idx)) != length(state_names)) {
...
#' @title Check if a Markov chain is exactly lumpable
#' @description Verifies whether a Markov chain is strongly lumpable with respect to a given partition.
#' @param object A \code{markovchain} object
#' @param partition A named list of character vectors representing the macro-states
#' @return A boolean value.
#' @references Kemeny, J. G., & Snell, J. L. (1960). Finite Markov Chains.
#' @export
...
setMethod("is.lumpable", signature(object = "markovchain"), 
          function(object, partition) {
            # Translate text partition to C++ indices
            part_idx <- .get_partition_indices(states(object), partition)
            # Delegate heavy lifting to C++
            return(.is_lumpable_cpp(object@transitionMatrix, part_idx))
          })

#' @title Aggregate a Markov chain
#' @description Coarses a Markov chain to a reduced state space given a partition.
#' @param object A \code{markovchain} object
#' @param partition A named list of character vectors representing the macro-states
#' @param force Boolean. If TRUE, forces approximate lumping even if the exact condition fails.
#' @return A \code{markovchain} object with the lumped transition matrix.
#' @export
...
setMethod("lump", signature(object = "markovchain"), 
          function(object, partition, force = FALSE) {
            ...
            # Check for exact lumpability unless the user explicitly forces the aggregation
            if (!force && !.is_lumpable_cpp(object@transitionMatrix, part_idx)) {
              stop("The Markov chain is not exactly lumpable. Use force = TRUE to perform an approximate weighted lumping.")
            }
            
            # Extract steady states to use as weights for the aggregation
            st <- steadyStates(object)
            if (nrow(st) > 0) {
              w <- st[1, ]
            } else {
              # If no steady state exists, use equiprobable weights
              w <- rep(1 / ncol(object@transitionMatrix), ncol(object@transitionMatrix))
            }
            
            # Delegate matrix multiplication and weighting to C++
            P_lumped <- .lump_cpp(object@transitionMatrix, part_idx, w)
            
            # Restore dimension names using the names provided in the partition list
...
#' @title Automatically lump a Markov chain using Spectral Clustering
#' @description Discovers macro-states and clusters them to find an approximate lumped Markov chain.
#' @param object A \code{markovchain} object
#' @param k Number of macro-states to discover
#' @return A list containing the partition and the lumped \code{markovchain} object.
#' @export
...
setMethod("autoLump", signature(object = "markovchain"), 
          function(object, k) {
            ...
            # The number of requested clusters must be mathematically viable
            if (k >= nrow(P) || k <= 1) {
              stop("The number of macro-states 'k' must be between 2 and the number of states - 1.")
            }
            
            # 1. Spectral Decomposition: Extract eigenvalues and eigenvectors
            eig <- eigen(P)
            
            # 2. Extract the right eigenvectors corresponding to the 'k' largest eigenvalues.
            # We use Re() to discard tiny imaginary parts caused by numerical noise.
            V_eig <- Re(eig$vectors[, 1:k])
            
            # 3. Apply K-Means clustering on the eigenvector space to find metastable basins
            set.seed(42) # Set seed to ensure the clustering results are deterministic
            clust <- kmeans(V_eig, centers = k, nstart = 25)
            
            # 4. Construct the partition list based on cluster assignments
            partition <- list()
            for (i in 1:k) {
              partition[[paste0("Macro_", i)]] <- state_names[clust$cluster == i]
            }
            
            # 5. Force the lumping using the newly discovered partition
            lumped_mc <- lump(object, partition, force = TRUE)
            
            return(list(
              partition = partition,
              lumped_chain = lumped_mc
            ))
          })
